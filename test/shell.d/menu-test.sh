...
assertEqual(menu.previewForRow({ kind: 'app', label: 'Firefox', appIcon: 'firefox' }).kind, 'app', 'menu previews applications through their icon')
assertEqual(menu.previewForRow({ kind: 'menu', label: 'Setup' }).kind, '', 'menu does not expand for non-previewable command rows')
assert(menuQml.includes('MenuModel.previewForRow(displayModel.get(index))'), 'menu derives previews from the selected result descriptor')
